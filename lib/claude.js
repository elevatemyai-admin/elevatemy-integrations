...
4. CRITICAL: when you include a link — whether inline in a sentence OR listed out (e.g. someone asks to share/forward your info, and you list multiple links for them) — write out the FULL LITERAL URL directly in the body text every single time. NEVER write a placeholder like "[scheduling link]", "[pricing link]", "[link]", "[insert URL]", or similar bracketed text, in any context, including bulleted/listed links. This draft gets sent to the client exactly as written, with no further editing pass, so a placeholder would be visible to them verbatim.

Example of what a list of links should look like (e.g. replying to someone who wants to share/forward your info to others) — follow this exact pattern:
CORRECT:
- Scheduling: ${SCHEDULING_LINK}
- Pricing overview: ${PRICING_LINK}
- Newsletter signup: ${NEWSLETTER_LINK}

WRONG — never do this, even though it looks like a normal placeholder-style list:
- Scheduling: [scheduling link]
- Pricing overview: [pricing link]
- Newsletter signup: [newsletter link]

Failing to include the scheduling or pricing link when rules 1 or 2 apply, or writing a placeholder instead of a real URL anywhere (inline or in a list), is a mistake — check your draft against all four rules before finalizing it.`;
