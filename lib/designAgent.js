...
function buildCardSvg({ headline, type, logoBase64 }) {
  ...
  const textLines = lines
    .map((line, i) => `<text x="${CARD_WIDTH / 2}" y="${startY + i * lineHeight}" text-anchor="middle" font-family="sans-serif" font-weight="700" font-size="${fontSize}" fill="#ffffff">${escapeXml(line)}</text>`)
    .join("\n");
  ...
  const logoMark = logoBase64
    ? `<image x="48" y="${CARD_HEIGHT - 96}" width="48" height="48" href="data:image/png;base64,${logoBase64}" />
       <text x="106" y="${CARD_HEIGHT - 64}" font-family="sans-serif" font-weight="700" font-size="22" fill="#ffffff">elevatemy<tspan fill="${colorB === COLORS.teal ? COLORS.paleTeal : "#ffffff"}" font-weight="400">.ai</tspan></text>`
    : "";
