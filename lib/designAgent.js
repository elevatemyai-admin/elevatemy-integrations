...
//      `sharp`. NOT a generative image model — this always looks
//      "on-brand" by construction, at the cost of every card sharing the
//      same layout/template rather than true visual variety.
//
// Requires the `sharp` npm package (added to package.json separately).
...
// character width as a fraction of font size for a bold sans-serif — not
// pixel-perfect, but good enough for headline-length text at this card
// size. If real font metrics ever matter more precisely, this is the
// piece to revisit first.
...
function buildCardSvg({ headline, type, logoBase64 }) {
  ...
  const textLines = lines
    .map((line, i) => `<text x="${CARD_WIDTH / 2}" y="${startY + i * lineHeight}" text-anchor="middle" font-family="Georgia, serif" font-weight="700" font-size="${fontSize}" fill="#ffffff">${escapeXml(line)}</text>`)
    .join("\n");
  ...
  const logoMark = logoBase64
    ? `<image x="48" y="${CARD_HEIGHT - 96}" width="48" height="48" href="data:image/png;base64,${logoBase64}" />
       <text x="106" y="${CARD_HEIGHT - 64}" font-family="Georgia, serif" font-weight="700" font-size="22" fill="#ffffff">elevatemy<tspan fill="${colorB === COLORS.teal ? COLORS.paleTeal : "#ffffff"}" font-weight="400">.ai</tspan></text>`
    : "";
