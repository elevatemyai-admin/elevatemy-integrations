...
// HONEST FLAG: the exact JSON field name this endpoint returns the count
// under could not be confirmed from available documentation. This logs
// the full raw response on every call so the correct field can be
// confirmed from a real response — check logs after the first real run
// and simplify the fallback chain below once confirmed.
async function getRegularCpaListCount() {
  if (!REGULAR_CPA_LIST_KEY) {
    console.warn("[zoho] ZOHO_LIST_KEY_CPA_REGULAR not set — cannot get list count");
    return null;
  }
  const token = await getZohoAccessToken();
  const params = new URLSearchParams({ resfmt: "JSON", listkey: REGULAR_CPA_LIST_KEY, version: "1" });
  const res = await fetch(`https://campaigns.zoho.com/api/listsubscriberscount?${params.toString()}`, {
    headers: { Authorization: `Zoho-oauthtoken ${token}` },
  });
  if (!res.ok) throw new Error(`Zoho Campaigns list-count error ${res.status}: ${await res.text()}`);
  const data = await res.json();

  console.log("[zoho] listsubscriberscount raw response (confirm field name against this):", JSON.stringify(data));

  const count = data.listsubscriberscount ?? data.count ?? data.no_of_active ?? data.subscribercount ?? null;
  if (count === null) {
    console.warn("[zoho] Could not find a recognized count field in listsubscriberscount response — returning null, caller should fall back to an estimate");
  }
