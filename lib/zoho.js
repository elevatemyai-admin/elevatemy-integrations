...
// getcampaignrecipientsdata caps out at PAGE_SIZE results per call (Zoho's
// docs put the practical max around 200) — it does NOT return everything in
// one shot the way the aggregate getcampaigndetails totals do. Any caller
// that hardcoded range=200/fromindex=1 with no loop was silently truncating
// at 200 recipients per sendKey, while the aggregate opens/clicks counts
// (from getcampaigndetails) stayed accurate and uncapped. That mismatch is
// exactly what surfaced as "dashboard opens vs. actual nurture-enrolled
// count don't agree" once a campaign passed 200 real recipients for a given
// action. This helper pages through fromindex until a page comes back
// shorter than PAGE_SIZE (Zoho's own signal that there's no more data), so
// every caller gets the true, complete list.
const RECIPIENTS_PAGE_SIZE = 200;
// Hard safety cap so a bug in the "shorter page = done" detection (or an
// unexpectedly huge list) can't turn this into an unbounded loop of live API
// calls — 50 pages at 200/page covers 10,000 recipients, comfortably above
// anything this project's lists are near today.
const RECIPIENTS_MAX_PAGES = 50;

async function fetchAllCampaignRecipients(sendKey, action) {
  const all = [];
  let fromindex = 1;
  for (let page = 0; page < RECIPIENTS_MAX_PAGES; page++) {
    const data = await zohoFetch(
      `https://campaigns.zoho.com/api/v1.1/getcampaignrecipientsdata?resfmt=JSON&campaignkey=${encodeURIComponent(sendKey)}&action=${action}&fromindex=${fromindex}&range=${RECIPIENTS_PAGE_SIZE}`
    );
    const list = data.list_of_details || [];
    all.push(...list);
    if (list.length < RECIPIENTS_PAGE_SIZE) break; // short page — this was the last one
    fromindex += RECIPIENTS_PAGE_SIZE;
  }
  return all;
}
...
async function fetchZohoCampaignClickers() {
  ...
  for (const c of campaigns) {
    ...
    for (const sendKey of c.sendKeys || [c.id]) {
      let list;
      try {
        list = await fetchAllCampaignRecipients(sendKey, "clickedcontacts");
      } catch (e) {
        console.warn(`[zoho] clicked recipients failed for campaign ${sendKey}:`, e.message);
        continue;
      }
      console.log(`[debug] CLICKED sendKey ${sendKey}: ${list.length} recipients returned (paginated)`);
...
async function fetchZohoCampaignOpeners() {
  ...
  for (const c of campaigns) {
    ...
    for (const sendKey of c.sendKeys || [c.id]) {
      let list;
      try {
        list = await fetchAllCampaignRecipients(sendKey, "openedcontacts");
      } catch (e) {
        console.warn(`[zoho] opened recipients failed for campaign ${sendKey}:`, e.message);
        continue;
      }
      console.log(`[debug] OPENED sendKey ${sendKey}: ${list.length} recipients returned (paginated)`);
      for (const entry of list) {
        const email = entry.contactemailaddress || entry.contact_email;
        if (!email) continue;
...
// Two separate lists (not one combined "engaged" list) because the
// opened-but-didn't-click and clicked segments get genuinely different
// follow-up content. Each of these lists should have its own "On List
// Entry" workflow in Zoho (On List Entry -> Message -> Wait -> Message,
// same pattern as the FP-Advanced tier lists) — NOT the paid Activity-based
// workflow type, which sidesteps the Professional-plan requirement.
const ENGAGEMENT_LIST_KEYS = {
  opened: process.env.ZOHO_LIST_KEY_CPA_OPENED_NURTURE,
  clicked: process.env.ZOHO_LIST_KEY_CPA_CLICKED_NURTURE,
};

// The list key for the current "regular" CPA cold-send list — contacts get
// REMOVED from this once they've engaged, so they stop receiving further
// cold sends once they're in a nurture path.
const REGULAR_CPA_LIST_KEY = process.env.ZOHO_LIST_KEY_CPA_REGULAR;

// Adds a contact to the opened- or clicked-nurture list. `kind` must be
// "opened" or "clicked". Silently skips if the corresponding env var isn't
// set yet, so this can be deployed before both lists/keys exist without
// breaking sync.
async function addContactToEngagementNurture(lead, kind) {
  const listkey = ENGAGEMENT_LIST_KEYS[kind];
  if (!listkey) {
    console.warn(`[zoho] ZOHO_LIST_KEY_CPA_${kind.toUpperCase()}_NURTURE not set — skipping nurture add for ${lead.email}`);
    return { skipped: true };
  }
  ...
  const contactinfo = JSON.stringify({
    ...
    "Last Name": lead.lastName || (lead.name || "").split(" ").slice(1).join(" ") || "",
    "Company Name": lead.company || "",
  });
  ...
  if (!res.ok) throw new Error(`Zoho Campaigns engagement-nurture add error ${res.status}: ${await res.text()}`);
  return res.json();
}

// Unsubscribes a contact from the regular cold-send CPA list. Uses
// listunsubscribe (not a hard delete — Zoho doesn't expose a documented
// single-contact delete endpoint). This is the "drop them from the list"
// step — stops them from receiving any future send to that list, without
// touching their membership on the nurture list added above.
async function removeContactFromRegularCpaList(email) {
  if (!REGULAR_CPA_LIST_KEY) {
    console.warn(`[zoho] ZOHO_LIST_KEY_CPA_REGULAR not set — skipping list removal for ${email}`);
    return { skipped: true };
  }

  const token = await getZohoAccessToken();
  const contactinfo = JSON.stringify({ "Contact Email": email });
  const params = new URLSearchParams({ resfmt: "JSON", listkey: REGULAR_CPA_LIST_KEY, contactinfo });
  const res = await fetch(`https://campaigns.zoho.com/api/v1.1/json/listunsubscribe?${params.toString()}`, {
    method: "POST",
    headers: { Authorization: `Zoho-oauthtoken ${token}` },
  });
  if (!res.ok) throw new Error(`Zoho Campaigns list-removal error ${res.status}: ${await res.text()}`);
  return res.json();
}

// Everyone EVER sent this campaign, regardless of what happened after
// (bounced, unsubscribed, opened, clicked, or just sat there unopened).
// This is the full exclusion set for the geo-replenishment job — per the
// "already contacted" definition, nobody who's ever been sent to should be
// treated as a fresh pull again, no matter which of the other four
// categories they landed in afterward.
// CONFIRMED action value against Zoho's current developer docs: "sentcontacts".
async function fetchZohoCampaignSentUniverse(campaignName) {
  const campaigns = await fetchZohoCampaigns();
  const match = campaigns.find((c) => c.name === campaignName);
  if (!match) throw new Error(`fetchZohoCampaignSentUniverse: no campaign found named "${campaignName}"`);

  const emails = new Set();
  for (const sendKey of match.sendKeys || [match.id]) {
    let list;
    try {
      list = await fetchAllCampaignRecipients(sendKey, "sentcontacts");
    } catch (e) {
      console.warn(`[zoho] sentcontacts failed for campaign ${sendKey}:`, e.message);
      continue;
    }
    for (const entry of list) {
      const email = entry.contactemailaddress || entry.contact_email || entry.leademailaddress;
      if (email) emails.add(email.toLowerCase().trim());
    }
  }
  return emails; // a Set, not an array — this can get large (1000s), Set dedup is worth it
}

// Adds a contact to the "regular" cold-send CPA list — the counterpart to
// removeContactFromRegularCpaList (see the engagement-nurture additions).
// Used by the list-health replenishment job when topping the active list
// back up with a new geo-sorted batch.
async function addContactToRegularCpaList(lead) {
  const listkey = process.env.ZOHO_LIST_KEY_CPA_REGULAR;
  if (!listkey) {
    console.warn(`[zoho] ZOHO_LIST_KEY_CPA_REGULAR not set — skipping regular-list add for ${lead.email}`);
    return { skipped: true };
  }
  const token = await getZohoAccessToken();
  const contactinfo = JSON.stringify({
  ...
  if (!res.ok) throw new Error(`Zoho Campaigns regular-list add error ${res.status}: ${await res.text()}`);
  return res.json();
}
...
// ---------------------------------------------------------------------
// Nurture-message engagement — did someone open/click the FOLLOW-UP email
// itself (the one Zoho's "On List Entry" workflow sends), as opposed to the
// original "CPAs Newsletter A/B" campaign. These are workflow-triggered
// sends, which recentcampaigns/fetchZohoCampaigns does NOT return — Zoho
// buckets them separately from manually-built campaigns even though they
// have their own report page and their own numeric report ID (visible in
// the report page's URL, e.g. .../reports/1670702000000171014/...).
//
// That report ID is a different format from a regular campaignkey (a long
// alphanumeric hash) — it's Zoho's standard numeric record-ID format.
// UNCONFIRMED whether it's actually valid as the `campaignkey` param on
// getcampaignrecipientsdata; every call here is wrapped so an invalid key
// just logs a warning and returns an empty list rather than breaking sync.
//
// Deliberately NOT merged into fetchZohoCampaignClickers/Openers or into
// upsertFromEngagement's re-enrollment logic — opening/clicking the nurture
// follow-up is a distinct signal from opening/clicking the original
// campaign, and should never re-trigger enrollment into a nurture list a
// contact is already on. Callers should log this as its own event type on
// engagementHistory (e.g. "nurture_open"/"nurture_click"), not feed it back
// into upsertFromEngagement/enrollInNurtureAndDropFromRegularList.
//
// Set via env vars once each workflow message has actually sent (a message
// with 0 sends yet has no report to pull from):
//   ZOHO_OPENED_NURTURE_MESSAGE_KEY
//   ZOHO_CLICKED_NURTURE_MESSAGE_KEY
const NURTURE_MESSAGE_KEYS = {
  opened: process.env.ZOHO_OPENED_NURTURE_MESSAGE_KEY,
  clicked: process.env.ZOHO_CLICKED_NURTURE_MESSAGE_KEY,
};

async function fetchNurtureMessageEngagement(kind) {
  const messageKey = NURTURE_MESSAGE_KEYS[kind];
  if (!messageKey) {
    console.warn(`[zoho] ZOHO_${kind.toUpperCase()}_NURTURE_MESSAGE_KEY not set — skipping nurture-message engagement fetch`);
    return { opens: [], clicks: [] };
  }

  async function fetchAction(action) {
    try {
      const list = await fetchAllCampaignRecipients(messageKey, action);
      console.log(`[debug] nurture-message ${kind} ${action}: ${list.length} recipients returned (key ${messageKey})`);
      return list
        .map((entry) => (entry.contactemailaddress || entry.contact_email || "").toLowerCase().trim())
        .filter(Boolean);
    } catch (e) {
      // Most likely cause if this always errors: messageKey isn't actually
      // a valid campaignkey for this endpoint — worth checking the raw
      // error text in logs before assuming the report ID format is right.
      console.warn(`[zoho] nurture-message ${kind} ${action} failed for key ${messageKey}:`, e.message);
      return [];
    }
  }

  const [opens, clicks] = await Promise.all([fetchAction("openedcontacts"), fetchAction("clickedcontacts")]);
  return { opens, clicks };
}

// =====================================================================
// Added for list-health.js: real bounce/optout counts and real list size
// =====================================================================
// Fixes two bugs found in the original list-health.js:
//   1. The guardrail check always passed since fetchZohoCampaigns() never
//      actually returned bounce/complaint counts — this reuses the same
//      fetchAllCampaignRecipients helper already proven working for
//      "sentcontacts" (see fetchZohoCampaignSentUniverse above), just with
//      different action values.
//   2. The deficit calculation approximated current list size as
//      sentUniverse.size, overestimating it since that never subtracts
//      people already removed — this gets the real, current count directly.

async function fetchZohoCampaignBounceAndOptoutSets(campaignName) {
  const campaigns = await fetchZohoCampaigns();
  const match = campaigns.find((c) => c.name === campaignName);
  if (!match) throw new Error(`fetchZohoCampaignBounceAndOptoutSets: no campaign found named "${campaignName}"`);
  ...
  for (const sendKey of match.sendKeys || [match.id]) {
    try {
      const bounceList = await fetchAllCampaignRecipients(sendKey, "senthardbounce");
      for (const entry of bounceList) {
        const email = entry.contactemailaddress || entry.contact_email || entry.leademailaddress;
        if (email) bounced.add(email.toLowerCase().trim());
      }
    } catch (e) {
      console.warn(`[zoho] senthardbounce failed for campaign ${sendKey}:`, e.message);
    }

    try {
      const optoutList = await fetchAllCampaignRecipients(sendKey, "optoutcontacts");
      for (const entry of optoutList) {
        const email = entry.contactemailaddress || entry.contact_email || entry.leademailaddress;
        if (email) optedOut.add(email.toLowerCase().trim());
      }
    } catch (e) {
      console.warn(`[zoho] optoutcontacts failed for campaign ${sendKey}:`, e.message);
    }
  }

  return { bounced, optedOut };
}

// CONFIRMED against Zoho Campaigns' own developer docs (v1.1 endpoint):
// https://campaigns.zoho.com/api/v1.1/listsubscriberscount?listkey=...&status=active
// returns { "status": "success", "code": "0", "no_of_contacts": N, "version": "1.1" }
//
// Uses status=active specifically because Zoho's own support confirmed
// bounced/unsubscribed contacts don't count against a plan's contact
// limit — so "active" is the number that actually matters for the
// 1,500-2,000 replenishment range, not total-ever-added.
//
// Generalized to take any list key (not just the regular list) — the
// 2,000 cap represents everyone currently active across the WHOLE CPA
// funnel, not just the regular outreach list. Someone mid-way through
// Opened-Nurture or Clicked-Nurture is still an active, billed contact,
// even though they're off the regular list by that point.
async function getZohoListActiveCount(listKey) {
  if (!listKey) return null;
  const token = await getZohoAccessToken();
  const params = new URLSearchParams({ resfmt: "JSON", listkey: listKey, status: "active" });
  const res = await fetch(`https://campaigns.zoho.com/api/v1.1/listsubscriberscount?${params.toString()}`, {
    headers: { Authorization: `Zoho-oauthtoken ${token}` },
  });
  if (!res.ok) throw new Error(`Zoho Campaigns list-count error ${res.status}: ${await res.text()}`);
  const data = await res.json();

  if (typeof data.no_of_contacts === "undefined") {
    console.warn(`[zoho] listsubscriberscount response missing expected no_of_contacts field for list ${listKey} — raw response:`, JSON.stringify(data));
    return null;
  }
  return Number(data.no_of_contacts);
}

// Sums active counts across the regular list AND both nurture lists —
// this is the number that actually matters for the 1,500-2,000 range,
// since all three count against the same account-wide contact limit.
// Returns null if ANY of the three lookups fails, so the caller falls
// back to an estimate rather than silently under-counting.
async function getTotalCpaFunnelActiveCount() {
  const keys = [
    REGULAR_CPA_LIST_KEY,
    process.env.ZOHO_LIST_KEY_CPA_OPENED_NURTURE,
    process.env.ZOHO_LIST_KEY_CPA_CLICKED_NURTURE,
  ];
  const counts = await Promise.all(keys.map((k) => getZohoListActiveCount(k)));
  if (counts.some((c) => c === null)) {
    console.warn("[zoho] one or more list counts failed — cannot compute a reliable total", counts);
    return null;
  }
  return counts.reduce((sum, c) => sum + c, 0);
}

// Kept for backward compatibility with anything still calling this
// directly — just the regular list's own count, not the combined total.
async function getRegularCpaListCount() {
  return getZohoListActiveCount(REGULAR_CPA_LIST_KEY);
}

// ---------------------------------------------------------------------
// Regular Outreach workflow messages — same root problem as nurture-
// message engagement: these are workflow-triggered sends, invisible to
// fetchZohoCampaigns()'s /recentcampaigns discovery. CONFIRMED via a live
// diagnostic (Aug 17, 2026): fetchZohoCampaigns() returned only 1 campaign
// total (the old one-time "CPAs Newsletter A/B" blast) — completely
// missing all three CPA – Regular Outreach messages, despite 28 real
// opens and 56 real bounces existing in Zoho's own report for Message 1
// alone.
//
// Unlike nurture-message engagement (informational-only logging, since
// those contacts already have CRM records by the time they reach nurture),
// Regular Outreach opens/clicks are often someone's FIRST signal — so
// this data is returned in the SAME shape as fetchZohoCampaignClickers/
// Openers, meant to be merged into those same arrays before
// upsertFromEngagement/enrollInNurtureAndDropFromRegularList run — giving
// these contacts the full treatment (new CRM record, tagging, nurture
// enrollment), not just an activity-log note.
//
// Report IDs found via each message's "View Detailed Report" URL
// (#automation/advworkflows/custom/{workflowId}/report/{reportId}) — same
// numeric-ID format already used for nurture-message tracking. Wrapped in
// try/catch same as everywhere else: an invalid ID just logs a warning
// and contributes nothing, rather than breaking sync.
const REGULAR_OUTREACH_MESSAGES = [
  { key: process.env.ZOHO_REGULAR_OUTREACH_MSG1_KEY, name: "What busy season actually costs you" },
  { key: process.env.ZOHO_REGULAR_OUTREACH_MSG2_KEY, name: "Not a case study — this is our own practice" },
  { key: process.env.ZOHO_REGULAR_OUTREACH_MSG3_KEY, name: "This was never really about the software" },
];

async function fetchRegularOutreachEngagement() {
  const openersByEmail = new Map();
  const clickersByEmail = new Map();

  for (const msg of REGULAR_OUTREACH_MESSAGES) {
    if (!msg.key) {
      console.warn(`[zoho] Missing report key for Regular Outreach message "${msg.name}" — set its env var to enable tracking`);
      continue;
    }

    const actions = [
      { action: "openedcontacts", map: openersByEmail, tsField: "openedAt", eventType: "open" },
      { action: "clickedcontacts", map: clickersByEmail, tsField: "clickedAt", eventType: "click" },
    ];

    for (const { action, map, tsField, eventType } of actions) {
      let list;
      try {
        list = await fetchAllCampaignRecipients(msg.key, action);
      } catch (e) {
        console.warn(`[zoho] Regular Outreach ${action} failed for "${msg.name}" (key ${msg.key}):`, e.message);
        continue;
      }
      console.log(`[debug] Regular Outreach ${action} for "${msg.name}": ${list.length} recipients returned (key ${msg.key})`);

      for (const entry of list) {
        const email = entry.contactemailaddress || entry.contact_email;
        if (!email) continue;
        const ts = entry.sent_time || entry.opened_time || entry.open_time || entry.clicked_time || null;
        const firstName = entry.contactfn && entry.contactfn !== "null" ? entry.contactfn : "";
        const lastName = entry.contactln && entry.contactln !== "null" ? entry.contactln : "";
        ...
        const record = existing || {
          ...
          company: entry.companyname && entry.companyname !== "null" ? entry.companyname : "",
          campaignName: msg.name, campaignLink: "", [tsField]: null,
          events: [],
        };
        record.events.push({ type: eventType, campaignName: msg.name, campaignLink: "", ts });
        if (!record[tsField] || (ts && ts > record[tsField])) {
          record[tsField] = ts;
          record.campaignName = msg.name;
        }
  ...
  return {
    openers: Array.from(openersByEmail.values()),
    clickers: Array.from(clickersByEmail.values()),
  };
}
...
module.exports = {
  ...
  fetchRegularOutreachEngagement,
  fetchNurtureMessageEngagement,
  addContactToNurtureList,
  addContactToDemoPitchList,
  addContactToEngagementNurture,
  ...
  getZohoListActiveCount,
  getTotalCpaFunnelActiveCount,
  contentToHtml,
  createCampaign,
  sendCampaignNow,
  scheduleCampaign,
  // Exported so api/marketing/approve.js can resolve a friendly tier name
  // (e.g. "Building") to its real Zoho list key server-side — the frontend
  // only ever sends/sees the tier name, never the actual key (an env var
  // value that must never reach the browser).
  TIER_LIST_KEYS,
};
