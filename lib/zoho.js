...
async function fetchZohoCampaignClickers() {
  ...
  for (const c of campaigns) {
    ...
    for (const sendKey of c.sendKeys || [c.id]) {
      ...
      console.log(`[debug] CLICKED sendKey ${sendKey}: ${list.length} recipients returned`);
...
async function fetchZohoCampaignOpeners() {
  ...
  for (const c of campaigns) {
    ...
    for (const sendKey of c.sendKeys || [c.id]) {
      ...
      const list = data.list_of_details || [];
      console.log(`[debug] OPENED sendKey ${sendKey}: ${list.length} recipients returned`);
      for (const entry of list) {
        const email = entry.contactemailaddress || entry.contact_email;
        if (!email) continue;
...
async function fetchZohoCampaignSentUniverse(campaignName) {
  ...
  for (const sendKey of match.sendKeys || [match.id]) {
    ...
    try {
      data = await zohoFetch(
        `https://campaigns.zoho.com/api/v1.1/getcampaignrecipientsdata?resfmt=JSON&campaignkey=${encodeURIComponent(sendKey)}&action=sentcontacts&fromindex=1&range=200`
      );
    } catch (e) {
      console.warn(`[zoho] sentcontacts failed for campaign ${sendKey}:`, e.message);
      continue;
    }
    for (const entry of data.list_of_details || []) {
