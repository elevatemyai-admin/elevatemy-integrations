...
async function getRegularCpaListCount() {
  ...
  const token = await getZohoAccessToken();
  const params = new URLSearchParams({ resfmt: "JSON", listkey: REGULAR_CPA_LIST_KEY, status: "active" });
  ...
  if (typeof data.no_of_contacts === "undefined") {
    console.warn("[zoho] listsubscriberscount response missing expected no_of_contacts field — raw response:", JSON.stringify(data));
    return null;
  }
