...
const { createCampaign, sendCampaignNow, scheduleCampaign } = require("../../lib/zoho");
...
module.exports = async (req, res) => {
  ...
  const { itemId, action, editedSubject, editedBody, scheduledFor } = req.body || {};
  ...
  try {
    ...
    if (item.type === "email") {
      if (!item.targetListKey) {
        items[idx] = { ...item, subject: finalSubject, body: finalBody, wasEdited, status: "approved", approvedAt: new Date().toISOString() };
        await setCache(CRM_DATA_KEY, crmData);
        return res.status(200).json({ ok: true, action: "approved", note: "Approved, but no target list is set yet — set one, then approve again to actually send." });
      }
      if (!SITE_BASE_URL) {
